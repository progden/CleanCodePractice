...
> 大類別壞味道是來自「沒有呈現好意圖」的 API
> 
> 發現這個壞味道後，如果是提供「提供細節作法」，應該要調整成「提供完成意圖」的API；
> 如果是「提供太多完成意圖的方式」則應該要「拆解類別」。

「提供細節作法」就是提供太多細節的 API，且都要呼叫過才能完成工作，也有可能會讓呼叫到一半的資料續存在系統中。
更關鍵的是，將如何完成工作的任務交給 Client 類別來決定，這也導致自身類別會是混亂零散的邏輯。就調整 API 吧。

「提供太多完成意圖的方式」就是太多種 client 的使用方式。拆解類別吧。讓每個類別只服務一類 Client 且只為了他們的的單一目的。

> 單一個職責可以不只依靠單一 API 來完成，所以並不是限制類別只能有一個 API

> 「太多職責，不夠單純」就是應該符合「物件導向」的「單一職責原則」。 
...
時常工程師遇到問題，東西做不完、事情重要不重要，回到SA或是PM的角色討論時，因為他們多知道更多的資訊，就變得好判斷了。
